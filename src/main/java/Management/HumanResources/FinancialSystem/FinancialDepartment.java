...
public class FinancialDepartment extends BaseDepartment {
    ...
    public void viewAuditHistoryList(){
        ...
        IOManager.getInstance().print(
                "*----当前版本："+ReportOriginator.getInstance().toString(IOManager.Lang.zh_CN),
                "*----當前版本："+ReportOriginator.getInstance().toString(IOManager.Lang.zh_TW),
                "*----Latest version: "+ReportOriginator.getInstance().toString(IOManager.Lang.en)
        );
        ...
        for(int i = 0; i<ReportAuditHistoryList.getInstance().getSize(); i++){
            ...
            IOManager.getInstance().print(
                    "*----No."+(i+1) +"："+ReportOriginator.getInstance().toString(IOManager.Lang.zh_CN),
                    "*----No."+(i+1) +"："+ReportOriginator.getInstance().toString(IOManager.Lang.zh_TW),
                    "*----No."+(i+1)+":" +ReportOriginator.getInstance().toString(IOManager.Lang.en)
            );
        }
